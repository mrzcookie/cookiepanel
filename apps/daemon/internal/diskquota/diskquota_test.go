...
func TestProjectIDDeterministicAndBounded(t *testing.T) {
	a := ProjectID("/var/lib/docker/volumes/cookied-srv-abc/_data")
	b := ProjectID("/var/lib/docker/volumes/cookied-srv-abc/_data")
	c := ProjectID("/var/lib/docker/volumes/cookied-srv-xyz/_data")
	if a != b {
...
func TestApplyNoToolIsNoOp(t *testing.T) {
	...
	enforced, err := Apply(context.Background(), "/tmp/cookied-quota-test", 64*1024*1024)
	if enforced {
