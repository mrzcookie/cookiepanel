...
import {
	type TemplateInputParsed,
	templateInputSchema,
	uniqueSlug,
} from "./validation";

/**
 * Templates service — the business logic shared by the org surface
 * (`server/templates`) and the admin/official surface (`server/admin/templates`).
 * It takes a *verified* `OwnerScope` + `Actor` (resolved from the session by the
 * thin server-fn shims, never the client) and:
 *  - projects DB rows to the client-safe `Template`, blanking raw image strings
 *    outside the write-gated editor (templates-over-images, security.md);
...
function toVariable(record: TemplateVariableRecord): TemplateVariable {
	return {
		id: record.id,
		name: record.name,
...
/** Group child rows by their templateId (preserving the repo's sort order). */
function groupBy<T extends { templateId: string }>(
	rows: T[]
): Map<string, T[]> {
...
export async function listTemplates(scope: OwnerScope): Promise<Template[]> {
	const records = await templatesRepository.list(scope);
	const ids = records.map((record) => record.id);
	const [images, variables] = await Promise.all([
		templatesRepository.imagesFor(ids),
		templatesRepository.variablesFor(ids),
	]);
...
/**
 * The full editor view of an owned template — raw image strings included. Null
...
export async function getEditableTemplate(
	...
	const record = await templatesRepository.findOwned(scope, id);
	if (!record) {
		return null;
	}
	const [images, variables] = await Promise.all([
		templatesRepository.imagesFor([id]),
		templatesRepository.variablesFor([id]),
	]);
	return toTemplate(record, images, variables, true);
}

// ─── input → row helpers ─────────────────────────────────────────────────────

/** Guarantee exactly one default runtime survives to the DB. */
function toImageValues(input: TemplateInputParsed): ImageValues[] {
...
function toVariableValues(input: TemplateInputParsed) {
	return input.variables.map((variable) => ({
		name: variable.name,
		description: variable.description,
		envVariable: variable.envVariable,
		// Secrets are write-only per-server; a template never stores a value.
...
function templateColumns(
	...
	slug: string
): Omit<TemplateValues, "origin" | "status" | "version" | "parentName"> {
	return {
		name: input.name,
		slug,
...
/** The origin recorded for a freshly-created template. */
function originFor(
	scope: OwnerScope,
	kind: "scratch" | "import" | "fork"
): TemplateRecord["origin"] {
	// An official template always reads as "Official", whatever the entry path.
...
async function insertTemplate(
	...
	const taken = await templatesRepository.listSlugs(scope);
	const slug = uniqueSlug(opts.slugSeed ?? input.name, taken);
	const record = await templatesRepository.create(
		scope,
		{
			...templateColumns(input, slug),
			origin: opts.origin,
			status: "draft",
			version: 1,
...
export function createTemplate(
	...
	return insertTemplate(scope, actor, input, {
...
/** Apply editor input to an owned template (full rewrite of its children). */
export async function updateTemplate(
	...
	const owned = await templatesRepository.findOwned(scope, id);
	if (!owned) {
		return null;
	}
	const taken = await templatesRepository.listSlugs(scope);
	...
	const updated = await templatesRepository.applyEdit(
		scope,
		id,
		templateColumns(input, slug),
		toImageValues(input),
	...
	await audit(actor, "template.updated", owned);
	return { ok: true };
}

/** Plain-language reasons a template can't be published — the server backstop. */
async function publishBlockers(record: TemplateRecord): Promise<string[]> {
	...
	const images = await templatesRepository.imagesFor([record.id]);
	if (images.length === 0) {
...
export async function publishTemplate(
	...
	await templatesRepository.update(scope, id, { status: "published", version });
	await audit(actor, "template.published", owned);
	return { ok: true, version };
}

/** Move a template back to draft (from published or archived). */
export async function unpublishTemplate(
	...
	await templatesRepository.update(scope, id, { status: "draft" });
	await audit(actor, "template.unpublished", owned);
	return { ok: true };
}

/** Take a template out of the catalog (existing servers unaffected). */
export async function archiveTemplate(
	...
	await templatesRepository.update(scope, id, { status: "archived" });
	await audit(actor, "template.archived", owned);
	return { ok: true };
}
...
export async function forkTemplate(
	...
	const source = await templatesRepository.findVisible(scope, sourceId);
	if (!source) {
		return null;
	}
	const [images, variables] = await Promise.all([
		templatesRepository.imagesFor([sourceId]),
		templatesRepository.variablesFor([sourceId]),
	]);
	const input = templateInputSchema.parse({
		...toTemplate(source, images, variables, true),
...
export async function importParsed(
	...
	const input = templateInputSchema.parse(parsed.input);
	const created = await insertTemplate(scope, actor, input, {
...
export async function deleteTemplate(
	...
	const removed = await templatesRepository.remove(scope, id);
	if (!removed) {
		return null;
	}
	await audit(actor, "template.deleted", owned);
	return { ok: true, refCount: 0 };
}

// ─── audit ───────────────────────────────────────────────────────────────────

function audit(actor: Actor, action: string, record: TemplateRecord) {
