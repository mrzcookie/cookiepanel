 ...
 * `eggInputSchema` in the service (the authoritative gate).
 */
...
const CATEGORY_SET = new Set<string>(EGG_CATEGORIES);

/** Best-effort category from the egg's name/description keywords. */
function guessCategory(text: string): EggCategory {
...
function normalizeCategory(raw: unknown, fallbackText: string): EggCategory {
	const value = str(raw);
	return CATEGORY_SET.has(value)
		? (value as EggCategory)
...
function normalizeVariable(
	raw: unknown,
	warnings: string[]
): EggInputParsed["variables"][number] | null {
...
function normalizeFeatures(raw: unknown): EggInputParsed["features"] {
...
export function parseEgg(raw: unknown): EggImportResult {
	...
	const name = (str(root.name).trim() || "Imported egg").slice(0, 120);
...
export async function fetchEggJson(rawUrl: string): Promise<unknown> {
	...
	try {
		...
		try {
			...
			throw new Error("That URL didn't return valid egg JSON.");
		}
	} finally {
		clearTimeout(timer);
	}
