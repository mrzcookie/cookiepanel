	...
	return (
		<>
			<PageHeader
				...
				description="Pair a Linux machine you own with CookiePanel. Name it, run one command on it, and it joins your fleet."
...
function stepHeading(stepId: StepId, name: string, connected: boolean) {
	...
	if (stepId === "setup") {
		return {
			...
			description:
				"Give it a name and choose how CookiePanel reaches it. You can rename it later.",
		};
	}
	if (stepId === "dns") {
		return {
	...
	return (
		<div className="max-w-xl space-y-5">
			<p className="text-muted-foreground text-sm">
				A node is a Linux machine you own that CookiePanel runs your servers on.
			...
			<p className="rounded-lg border bg-muted/20 px-3 py-2.5 text-muted-foreground text-xs">
				Nothing else to install. CookiePanel sets up everything it needs on the
		...
		<div className="max-w-xl space-y-6">
			...
			<div className="space-y-2.5">
				...
				<fieldset className="grid gap-3 sm:grid-cols-2">
					<legend className="sr-only">How CookiePanel reaches this node</legend>
					<ModeTile
		...
		<div className="space-y-6">
			...
			<section className="space-y-3 border-t pt-5">
				...
				<p className="text-muted-foreground text-sm">
					...
					203.0.113.24. CookiePanel does not know it yet, so grab it from the
			...
			<div className="border-t pt-5">
				<Collapsible summary="// also make sure your ports are open">
					...
					<ul className="space-y-1">
						<li>
							Port {port || "8443"} (TCP) is how CookiePanel talks to your node.
						</li>
						<li>
