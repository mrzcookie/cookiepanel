...
import { VariableEditorDialog } from "@/components/eggs/variable-editor-dialog";
	...
	async function save() {
		...
		try {
			if (mode === "create") {
				...
				navigate({
					params: { eggId: created.id },
					to: scope.editPath,
				} as never);
			} else if (eggId) {
				await actions.update(eggId, input);
				await invalidateEggs(queryClient);
				toast.success("Changes saved.");
			}
		} catch {
			toast.error("Couldn't save the egg. Try again.");
		} finally {
			setSaving(false);
		}
	}
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-end gap-2 border-t pt-4">
				<Button asChild variant="ghost">
					{mode === "edit" && eggId ? (
						<Link params={{ eggId } as never} to={scope.detailPath as never}>
							Cancel
						</Link>
					) : (
				...
				<Button disabled={saving} onClick={save}>
					{mode === "create" ? "Create egg" : "Save changes"}
				</Button>
			</div>
		</div>
	);
	...
	return (
		<div className="max-w-2xl space-y-4">
			...
			<div className="grid gap-2">
				...
				<Select
					onValueChange={(value) => patch({ category: value as EggCategory })}
			...
			<div className="grid gap-2">
				...
				<Textarea
					...
					placeholder="What this egg runs, and anything players should know."
					rows={5}
	...
	return (
		<div className="max-w-3xl space-y-4">
			<p className="text-muted-foreground text-sm">
				...
				server is set up from this egg. Use it to download server files into{" "}
				<span className="font-mono text-xs">/mnt/server</span>. Leave it blank
				for runtimes that need no setup.
			</p>
...
function AddonsTab({ state, patch }: TabProps) {
	...
	return (
		<div className="max-w-3xl space-y-4">
			<p className="text-muted-foreground text-sm">
				Extra tools players get for servers built from this egg. Turn on the
				ones that apply.
			</p>
			...
			{unknown.length > 0 ? (
				<div className="space-y-2">
					<p className="text-muted-foreground text-xs">
						Capabilities declared on this egg that the panel has no module for.
						They're kept as-is; remove any you don't want.
					</p>
