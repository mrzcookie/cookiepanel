...
// the way to the store's `EggInput`.
...
/**
 * Hydrate the editor from an existing egg. Mints fresh ids for rows that
 ...
 */
export function eggToState(egg: Egg): EditorState {
	return {
		name: egg.name,
		summary: egg.summary,
		...
		stopType: egg.stopType,
		stopValue: egg.stopValue,
		doneMarkers: egg.doneMarkers.map((matcher) => ({
		...
		configFiles: egg.configFiles.map(configFileToEditor),
	};
}

/** Expand a stored config file's `replace` map into id-keyed editor entries. */
function configFileToEditor(file: EggConfigFile): EditorConfigFile {
	return {
		id: crypto.randomUUID(),
		file: file.file,
		parser: file.parser,
 ...
 */
function editorToConfigFiles(files: EditorConfigFile[]): EggConfigFile[] {
...
export function stateToInput(state: EditorState): EggInput {
	return {
