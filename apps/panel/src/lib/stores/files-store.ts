import type { FileNode } from "@/lib/domain/files";
import { basename, joinPath, parentPath } from "@/lib/domain/files";
...
const store = createStore<Record<string, FileNode[]>>(seedTrees());
...
export function useServerFiles(serverId: string): FileNode[] {
	return store.use()[serverId] ?? EMPTY;
}

function setTree(serverId: string, next: FileNode[]) {
	store.set({ ...store.get(), [serverId]: next });
}

// — Mutations —————————————————————————————————————————————————————————————————

function addNode(serverId: string, entry: FileNode) {
	const current = store.get()[serverId] ?? EMPTY;
