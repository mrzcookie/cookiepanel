...
//
// Each server's data lives in a docker named volume "wings-srv-<id>" (created
...
//
//	.raptor-trash/<id>/payload    the file or directory that was deleted
//	.raptor-trash/<id>/meta.json  its original path, name, and deletion time
//
// User-facing file ops (List/Read/Write/...) treat this directory as if it
// doesn't exist; only the Trash* methods reach inside it.
const trashDirName = ".raptor-trash"
