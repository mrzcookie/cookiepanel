// Package tui is wings's offline operator console: a local terminal UI to list
...
func titleBar(section string, w int) string {
	head := styleBrand.Render("WINGS") + styleDim.Render("  ·  "+section)
	return head + "\n" + rule(w)
}
...
func (m model) statusEyebrow() string {
	...
	parts := fmt.Sprintf("// node %s · wings %s · %d server%s, %d running",
